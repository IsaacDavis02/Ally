...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Ally_SNAI").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Ally_SNAI").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Ally_SNAI", 20, None).await?;
